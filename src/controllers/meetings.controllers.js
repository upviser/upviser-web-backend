...
export const CreateMeeting = async (req, res) => {
    try {
        if (req.body.type === 'Llamada por Zoom') {
            ...
            const storeData = await StoreData.find()
            const style = await Style.find()
            await sendEmailBrevo({ subscribers: [{ name: req.body.firstName, email: req.body.email }], emailData: { affair: `¡Hola ${req.body.firstName}! Tu visita ha sido agendada con exito`, title: 'Hemos agendado tu visita exitosamente', paragraph: `¡Hola ${req.body.firstName}! Te queriamos informar que tu visita con fecha ${new Date(req.body.date).getUTCDate()}/${new Date(req.body.date).getUTCMonth() + 1}/${new Date(req.body.date).getUTCFullYear()} a las ${new Date(req.body.date).getUTCHours()}:${new Date(req.body.date).getUTCMinutes() >= 9 ? new Date(req.body.date).getUTCMinutes() : `0${new Date(req.body.date).getUTCMinutes()}`} ha sido agendada con exito, la visita sera en ${req.body.type === 'Visita a domicilio' ? `${req.body.address}, ${req.body.city}, ${req.body.region}.` : `${storeData[0].address}, ${storeData[0].city}, ${storeData[0].region}.`} Para cualquier consulta comunicate con nosotros a través de nuestro Whatsapp.`, buttonText: 'Hablar por Whatsapp', url: `https://wa.me/+56${storeData[0].phone}` }, clientData: clientData, storeData: storeData[0], style: style[0] })
        }
    } catch (error) {
        console.log(error)
        return res.status(500).json({message: error.message})
